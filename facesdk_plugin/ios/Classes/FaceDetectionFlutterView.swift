...
// Or whatever your SDK's module name is
...
class FaceDetectionFlutterView : NSObject, FlutterPlatformView, AVCaptureVideoDataOutputSampleBufferDelegate, AVCapturePhotoCaptureDelegate{
    ...
    func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
       ...
       if(Date().timeIntervalSince1970 - startTime <= 0.5) {
        ...
        // param.checkLiveness = true  // Adjust based on SDK
        // param.checkLivenessLevel = FaceDetectionFlutterView.livenessDetectionLevel
        ...
        param.check_face_occlusion = true   // Check for face obstructions
        param.check_mouth_opened = false    // Disable mouth open check
        param.estimate_age_gender = true // Use your actual level variable
    
        let faceBoxes = FaceSDK.faceDetection(capturedImage, param: param)
        let faceBoxesMap = NSMutableArray()
        ...
        DispatchQueue.main.async {
            let faceBoxesArray = faceBoxesMap as Array
